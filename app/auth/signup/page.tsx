...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else if (formData.confirmPassword.length < 6) newErrors.confirmPassword = "Confirm password must be at least 6 characters";
    ...
    try {
      ...
      console.error(error);
      const msg = error.response?.data?.message?.[0] || error.response?.data?.error || error.message || "An unexpected error occurred";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main className="flex min-h-screen w-full items-center justify-center bg-[#F8F9F5] font-sans antialiased">
      ...
      <div className="flex w-full max-w-[1440px] min-h-screen lg:min-h-[1004px] overflow-hidden bg-[#F8F9F5] relative flex-col lg:flex-row">
        ...
        <div className="relative hidden w-full lg:w-[45%] xl:w-[728px] lg:min-h-screen xl:h-[1004px] overflow-hidden lg:block flex-shrink-0 z-0">
          <Image
            src="/signupframe.png"
