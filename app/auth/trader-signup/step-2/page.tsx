...
export default function Step2Page() {
        ...
        getRegistrationStatus().then(res => {
            ...
            if (unwrapped?.traderData) {
                setFormData(prev => ({
                    ...
                    skillServiceId: unwrapped.selectedSkillServices?.[0]?.id || "",
                    subCategoryId: unwrapped.selectedSubCategories?.[0]?.id || "",
                    expMin1Year: true,
                    authorized: true,
                    vettingTerms: true,
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.skillServiceId.trim()) {
            toast.error("Please select a Skill Service.");
            return;
        }
        ...
        if (!isUpdateMode && (!idFile || !logoFile)) {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                    ...
                    <div className="relative">
                        <select
                            ...
                            className={`${inputCls} appearance-none pr-10 cursor-pointer bg-white ${!formData.skillServiceId ? "text-[#1C2C1C]/40" : "text-[#1C2C1C]"
                                }`}
                        >
                    ...
                    <div className="relative">
                        <select
                            ...
                            className={`${inputCls} appearance-none pr-10 cursor-pointer bg-white ${!formData.subCategoryId ? "text-[#1C2C1C]/40" : "text-[#1C2C1C]"
                                } disabled:opacity-50`}
                        >
