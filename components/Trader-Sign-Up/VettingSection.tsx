    ...
    <section id="vetting-section" className="py-16 px-4 font-sans bg-[#F8F9F7]">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <p className="text-[#0A2619B2] text-lg max-w-2xl leading-relaxed">
            To maintain quality on our platform, all traders go through a vetting<br />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10 max-w-[860px] mx-auto">
          {vettingSteps.map((step, index) => (
            <div
              key={index}
              className="bg-[#D6DED0] border border-[#C4CEBE] rounded-[22px] p-5 flex flex-col items-center justify-between min-h-[210px] shadow-sm hover:shadow-md transition-all"
            >
              <div className="bg-white p-3 rounded-[16px] shadow-sm mb-4 mt-1">
                {step.icon}
              </div>

              <h4 className="text-[16px] font-bold text-[#0A2619] text-center px-1 leading-snug">
                {step.title}
              </h4>

              <div className="mt-auto mb-1">
                <div className="w-7 h-7 bg-[#0A26190D] border border-[#0A26191A] rounded-full flex items-center justify-center">
                  <Check className="w-5 h-5 text-[#0A2619] opacity-40" />
                </div>
              </div>
            </div>
          ))}
